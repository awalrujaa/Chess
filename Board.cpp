...
Board::Board()
{
	if (!backText.loadFromFile("back.png"))
	{
		std::cout << "cant Load background" << std::endl;
	}
	backgroundImage.setTexture(backText);
	backgroundImage.setPosition(50, 50);
...
bool Board::movePawn(Square* thisPawn, Square* thatSpace) {
	...
	if (thisPawn->getColor() == WHITE)
	{
		...
		if (pawnX == thatX && thatY == pawnY + 1 && thatSpace->getColor() == NONE)
		{
			...
			if (thatSpace->getY() == 7 || thatSpace->getY() == 0) {
				thatSpace->promote(thisPawn);
				thisPawn->setEmpty();
			}
			else {
